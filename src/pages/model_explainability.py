...
def render_explainability_page():
    ...
    if explainability_data:
        ...
        st.warning("Explainability data not available. Please ensure the model explainability analysis has been completed.")
...
def render_feature_importance_overview(explainability_data):
    ...
    with tab1:
        render_classification_feature_importance(explainability_data["feature_importance"]["classification"])
...
def render_individual_predictions(explainability_data):
    ...
    st.markdown("Enter your health information to see how the model would make a prediction for you.")
    ...
    with st.form("individual_prediction"):
        ...
        if st.form_submit_button("🔍 Explain Prediction", use_container_width=True):
            ...
            for feature, shap_value in prediction_result["shap_values"].items():
                ...
                st.markdown(f"• **{feature.upper()}**: {impact} the likelihood of {prediction_result['prediction']} (impact: {shap_value:.2f})")
