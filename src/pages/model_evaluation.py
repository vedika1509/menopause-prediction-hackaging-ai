...
def render_model_evaluation_page():
    ...
    if model_insights:
        ...
        st.warning(
            "Model insights not available. Please ensure the model evaluation has been completed."
        )
...
def generate_sample_model_insights():
    ...
    return {
        "classification": {
            ...
            "roc_auc": 0.92,
            "feature_importance": {"age": 0.35, "fsh": 0.28, "amh": 0.22, "bmi": 0.15},
        },
        "survival": {
            "c_index": 0.78,
            "mae": 2.3,
            "rmse": 3.1,
            "feature_importance": {"age": 0.42, "fsh": 0.31, "amh": 0.18, "bmi": 0.09},
        },
        "symptom": {
            "mae": 1.2,
            "rmse": 1.8,
...
def render_classification_metrics(classification_data):
    ...
    with col2:
        ...
        fig.add_trace(
            go.Scatter(
                x=fpr, y=tpr, mode="lines", name="ROC Curve", line=dict(color="#9B59B6", width=3)
            )
        )
